...
# When we create a new Load Balancer
...
# When we use an existing load balancer
...
resource "ibm_is_lb_pool" "lb_pools" {
  ...
  lb                              = var.create_load_balancer ? ibm_is_lb.lbs[0].id : data.ibm_is_lb.lbs[0].id
...
resource "ibm_is_lb_pool_member" "lb_members" {
  for_each       = { for l in var.lb_pool_members : l.name => l }
  lb             = var.create_load_balancer ? ibm_is_lb.lbs[0].id : data.ibm_is_lb.lbs[0].id
